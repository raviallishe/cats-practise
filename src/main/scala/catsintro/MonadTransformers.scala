...
//  def sumAllOptions(values: List[Option[Int]]): Int = ???
  // with regular approach, have to unwrap and transform and then again wrap which makes a code a bit not readable

  // option transformer are good for nested types
